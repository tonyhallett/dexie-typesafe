...
type CompoundType<T extends readonly any[]> = T extends readonly [infer Only] ? Only : T;
  ...
  ? Values extends [infer V, ...infer RestValues]
    ? AccObj & { [P in K]: V } extends infer NextObj extends object
      ? CompoundKeyLookupArray<
          ...
          NextObj,
          [...AccTuple, V],
          [...Out, EqualityKeyType<Flatten<NextObj>, CompoundType<[...AccTuple, V]>>]
        >
      : Out
    : Out
  : Out;
...
type PKValueTuple<TPkey extends any | readonly any[]> = TPkey extends readonly any[]
  ? TPkey
  : never;
...
> = TPaths extends readonly [infer H, ...infer R extends DexieIndexPaths<TDatabase>]
  ? H extends { path: infer P extends string; [KeyTypeBrand]?: infer K }
    ? IndexKeyTypesSplit<
        TDatabase,
        R,
        {
          single: readonly [...Out["single"], EqualityKeyType<{ [KPath in P]: K }, K>];
    ...
    : H extends {
      ...
      ? IndexKeyTypesSplit<
          ...
          {
            single: Out["single"];
            composite: readonly [...Out["composite"], ...CompoundKeyLookupArray<PS, KS>];
          }
        >
...
type PrimaryKeyTypes<TPKeyPathOrPaths, TPkey extends any | readonly any[]> = {
  single: TPKeyPathOrPaths extends string ? PkEqualityEntries<TPKeyPathOrPaths, TPkey> : [];
  composite: TPKeyPathOrPaths extends readonly string[]
    ? PkEqualityEntries<TPKeyPathOrPaths, TPkey>
    : [];
};

type ComputePrimaryKeyTypes<TPKeyPathOrPaths, TPkey extends any | readonly any[]> = PrimaryKeyTypes<
  TPKeyPathOrPaths,
  TPkey
>;
...
> =
  ComputePrimaryKeyTypes<TPKeyPathOrPaths, TPkey> extends infer PKs extends SplitEqualityKeyTypes
    ? IndexKeyTypesSplit<TDatabase, TPaths> extends infer IndexKeys extends SplitEqualityKeyTypes
...
type UnionOfMatchIndexTypes<TMatch extends Record<string, any>, TIndexLookup> =
  TIndexLookup extends Record<string, any>
    ? TIndexLookup[keyof Pick<TIndexLookup, keyof TMatch & keyof TIndexLookup>]
    : never;

type CorrectPropertyTypes<TEquality, TMatch> = keyof TMatch extends keyof TEquality
  ? false extends {
...
type IndexPropertyIsNever<TIndexProperty> = [TIndexProperty] extends [never] ? true : false;

type Condition<TEquality, TMatch, TIndexProperty extends keyof TEquality> =
...
> = T extends readonly [infer First, ...infer Rest]
  ? First extends EqualityKeyType<infer Equality, infer KeyType, infer IndexProperty>
...
type EqualityFilterPaths<T, TMaxDepth extends string, TCurrDepth extends string = NoDescend> = {
  [P in keyof T]: P extends string
    ? TCurrDepth extends TMaxDepth
      ? P
      : T[P] extends Record<string, unknown>
        ? P | `${P}.${EqualityFilterPaths<Required<T[P]>, TMaxDepth, NextDepth<TCurrDepth>>}`
        : P
    : never;
}[keyof T];
type EqualityFilter<T, TMaxDepth extends string> = {
  [KP in EqualityFilterPaths<Required<T>, TMaxDepth>]?: KeyPathValue<Required<T>, KP>;
};
