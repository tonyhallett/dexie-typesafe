...
type IsMethod<T, Property extends keyof T> = T[Property] extends (
  ...
  ? {
      [key in keyof T]: key extends Property ? (arg0: never, ...args: Rest) => R : T[key];
    } extends T
    ? true
    : false
  : false;

type ZeroParameters<F extends (...args: any) => any> = Parameters<F> extends [] ? true : false;

type NoExcessDataPropertiesIsPotentialMethod<T, Property extends keyof T> = T[Property] extends (
  ...args: any[]
) => any
    ...
    : // Arrays / tuples
      ...
      : // Map
        ...
        ? TExtended extends Map<infer UK, infer UV>
          ? Map<NoExcessDataProperties<K, UK, TLeaves>, NoExcessDataProperties<V, UV, TLeaves>>
          : TExtends
        : // Set
...
> = TExtended extends unknown ? NoExcessDataPropertiesImpl<TExtends, TExtended, TLeaves> : never;
...
> = TArr extends readonly [infer First, ...infer Rest]
  ? First extends TInsert
    ? HasNeverProperty<NoExcessDataProperties<First, TInsert, TLeaves>> extends never
      ? Rest extends readonly []
        ? readonly [First]
        : readonly [First, ...NoExcessDataPropertiesArray<Rest, TInsert, TLeaves>]
      : never
    : never
  : readonly [];
...
export type IncludesNumberInUnion<T> = Extract<T, number> extends never ? false : true;
...
export type NoDuplicates<T extends readonly any[]> = T extends readonly [infer First, ...infer Rest]
...
export type First<T extends readonly any[]> = T extends readonly [infer A, ...any[]] ? A : never;

export type PathKeyType<TPath extends string | readonly string[], TKey> = {
  readonly path: TPath;
  readonly keyType: TKey;
};
export type PathKeyTypes = readonly PathKeyType<string | readonly string[], any>[];

// Conditional wrappers to optionally enforce NoExcessDataProperties
export type MaybeNoExcess<T, TInsert, TLeaves, TDisabled extends boolean> = TDisabled extends true
  ? T
  : NoExcessDataProperties<T, TInsert, TLeaves>;
...
type UnionToTupleRec<U, R extends readonly any[] = readonly []> = [U] extends [never]
  ? R
...
export type ExcludeKeysTuple<T, K extends keyof T> = UnionToTuple<ExcludeKeys<T, K>>;
