...
export type MultiIndexPath<T, P extends string> = {
  path: P;
  multi: true;
  [KeyTypeBrand]?: KeyPathValue<T, P> extends readonly (infer Elem)[] ? Elem : KeyPathValue<T, P>;
};
...
type Accumulate<Brand extends readonly any[], Paths extends readonly string[]> = Paths extends [
...
> = TIndexPaths extends readonly [infer H, ...infer Rest]
  ? H extends DexieIndexPath<TDatabase>
    ? Rest extends readonly DexieIndexPath<TDatabase>[]
      ? readonly [...PathWithKeysForDexieIndexPath<H>, ...IndexPathRegistry<TDatabase, Rest>]
      : []
    : []
  : [];

export type IndexPath<T, I extends DexieIndexPath<T>> =
